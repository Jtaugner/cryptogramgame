...
shuffleArray(locationLevels);

locationLevels.forEach(obj => {
     let text = obj.text;
     text = replaceLongDashes(text.replace(/ё/gi, 'е'));
     if(/[a-z]/ig.test(text)){
          return;
...
fsp.writeFile(path.join(__dirname, 'levelsList.js'),
 ...
 fsp.writeFile(path.join(__dirname, 'locationLevelsList.js'),
 `const locationLevelsList = [
